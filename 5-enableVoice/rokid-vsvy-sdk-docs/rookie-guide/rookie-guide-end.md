...
**注意：**【方案类型】分为 [**全链路通用方案**](./fullLink/fulllink.md) 和 [**基础语音模块**](./speechTTS/speechtts.md) 两种。两者的主要区别是：全链路方案会包含前端模块（拾音方式和MIC阵列），基础语音模块不包含前端模块（拾音方式和MIC阵列）。<font color=red>眼镜产品默认选择[**全链路通用方案**](./fullLink/fulllink.md) 。</font>
...
<font color=red>眼镜产品默认选择线性二麦 。</font>
