...
* [一、创建和配置产品](#一、创建和配置产品)
  ...
  * [配置产品](#配置产品)
* [二、获取SDK](#二、获取sdk)
* [三、获取测试序列号和导入序列号](#三、获取测试序列号和导入序列号)
  * [获取测试序列号](#获取测试序列号)
  * [导入正式序列号](#导入正式序列号)
...
序列号（同device id）即为设备的序列号，设备的唯一标识项。用于进行Rokid设备认证和Rokid平台服务认证的基础参数。测试序列号，主要用于测试期间使用Rokid设备认证和获取平台服务。

##### 序列号和typeID的区别

typeID是同一类型硬件产品的共有id，而序列号则为该类型产品下单个设备的唯一id。技能授权仅与typeID有关，与设备序列号无关，用户提供typeID获取了技能授权后，该typeID下的所有设备均可使用这些技能。

> 如：Rokid Pebble的typeID为95XXXXXX，但每一台pebble都有自己的序列号。

##### 获取测试序列号的方式

测试序列号和Seed不再人工提供，用户在https://developer.rokid.com/voice/ 注册后，创建产品，进行自动生成。可在线进行查看和获取，具体有如下两种获取方式：

###### 方式一

进行SDK下载时点击获取测试序列号，如下图所示：

![](images/sn01.png)
###### 方式二

产品管理页面顶部设备序列号，筛选对应产品即可看到测试序列号，如下图所示：
...
测试完成后，若想正式使用Rokid服务，需要导入正式序列号。

正式序列号需要联系商务（商务邮箱：rokidopen@rokid.com）签订合同后才能获取导入资格。**邮件内容请写明：公司名称、联系方式、申请导入序列号数量。**商务授权后即可导入正式序列号。导入正式序列号后，Rokid开放平台即对这批序列号设备进行授权，这批设备即可正式使用Rokid语音交互服务。

##### 具体操作

进入【设备序列号】页面，然后点击【导入序列号】，并按照指定模板导入即可。如下图所示：
...
导入完成后，页面会提示导入成功。10分钟后，该批序列号即可使用平台的语音服务。客户可以在页面的【类别选择】中选择【正式】类别查看自己导入的序列号号。

![](images/sn05.png)



**注意：**若导入的序列号号有重复，系统会自动对其进行去重，重复的序列号号不计算在总的序列号数量里。
