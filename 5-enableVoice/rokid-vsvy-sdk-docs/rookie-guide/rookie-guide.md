...
本文介绍如何通过Rokid开放平台获取SDK、配置产品技能和聊天以及进行系统管理。
...
* [三、设备SN导入和管理](#三、设备SN导入和管理)
  ...
  * [导入正式SN](#导入正式SN)
* [四、系统发布服务](#四、系统发布服务)
* [五、认证文件使用方法](#五、认证文件使用方法)
  * [直接调用API时的配置方法](#直接调用api时的配置方法)
  * [使用C++ SDK时的配置方法](#使用c++sdk时的配置方法)
...
要使用Rokid开放平台的语音整体方案，请先点击[Rokid开放平台](https://developer.rokid.com/#/)首页右上方的【登录】按钮，登录Rokid开放平台。如果未有账号，则点击旁边的【注册】按钮进行注册。如下图所示。

![](images/01.png)

登录以后，即进入了Rokid开放平台的控制台。首先您需要勾选【同意并接受该协议所有内容】以同意开发者社区服务协议，并点击确定。如下图所示。

![](images/fuwuxieyi.jpg)

然后选择语音整体方案，点击【立即接入】，即可开始使用语音接入工具。如下图所示。
...
产品是指您想要接入Rokid语音服务的一种设备，一个产品只对应一种语音配置。当您想要实现多种语音配置效果的时候，需要创建多个产品。
...
接下来，为了丰富设备的功能，可以按照自己产品的特性选择平台上面丰富的技能。
...
<table>
    ...
    <tr>
        ...
        <td width="70%" word-wrap: break-word;word-break:break-all;>私有技能无法向所有用户开放，需要申请授权，平台操作授权后，才能正常使用，需要联系商务（rokidopen@rokid.com)进行申请</td>
    </tr>
</table>
...
【我创建的技能】为当前账号创建的已发布技能，若您想要添加自己创建
的技能， 若您的技能是私有技能，您需要技能创建的发布页面，添加设备typeid，然后前往语音接入的服务配置页面，接受授权，若为公有技能，直接在语音接入的服务配置页面添加自己创建的技能 。
...
- 页面上展示的Rokid自有技能，您可以在页面上提出申请，由商务(商务邮箱:rokidopen@rokid.com)同意授权后即可使用。
...
![](images/126.png)
点击后会弹出如下页面，接下来就可以进行下载SDK，获取测试序列号，进行产品的在线测试等操作。如下图所示。
...
SN为产品的序列号，即设备ID(device_id),设备的唯一标识项，是进行Rokid设备认证和Rokid平台服务认证的基础参数。产品使用Rokid语音服务，需要进行SN导入。导入成功后，即该SN对应的设备成功授权，设备可以正式使用Rokid语音交互服务。
SN为厂商根据自己的实际情况来定义，支持数字、字母，且字母区分大小写（比如SN00001和sn00001，为两个不同的sn号）。

测试SN，主要用于测试期间使用Rokid设备认证和获取平台服务。
平台提供10个免费测试SN用于产品验证阶段测试使用，正式发布请联系Rokid(rokidopen@rokid.com)申请正式自有SN的导入权限。


- 设备TypeID和设备ID(device_id，即SN)的区别

设备typeID是同一类型硬件产品的共有id，而设备ID则为该类型产品下单个设备的唯一id。技能授权仅与typeID有关，与设备id无关，用户提供typeID获取了技能授权后，该typeID下的所有设备均可使用这些技能。

如：Rokid Pebble的typeID为95XXXXXX，但每一台pebble都有自己的id（sn号）。

- 什么是 seed，在什么情况下需要使用 seed
...
###### 每个测试sn号对应的seed，可以通过点击进入【导入记录】来下载。
...
正式序列号需要联系商务（商务邮箱：rokidopen@rokid.com）签订合同后才能获取导入资格。**邮件内容请写明：公司名称、联系方式、申请导入SN数量。**商务授权后即可导入正式 SN。导入正式序列号后，Rokid 开放平台即对这批 SN 设备进行授权，这批设备即可正式使用 Rokid 语音交互服务。

- 具体操作

进入【产品列表】页面，然后进入想要导入SN的产品页面，点击【设备SN】，并按照指定模板导入即可。如下图所示：
...
导入完成后，页面会提示导入是否成功。同时，在【导入记录】中也会新增每次导入的记录和错误反馈。在10分钟后，该批设备即可使用平台的语音服务。客户可以在页面的【类别选择】中选择【正式】类别查看自己导入的sn号。
...
B端用户可以根据不同的设备接入相同的人工语料引擎的同时，可以通过人设引擎进行自定义差异化，同时也可以自主选择是否接入Rokid 百科文档引擎和自动聊天引擎。
...
为你的设备配置问答内容。用户说出你配置的语句后，设备将你配置的问句对应的回答回复给用户。 如果已配置的回答语句超过1个，则随机回答。
...
开启后，设备可以自动回复用户的闲聊语句
聊天兜底语句，为设备配置兜底语句。当自动闲聊无法回答用户问题时，将回复用户兜底语句中的内容。已为你预置了一些内容，也可以自行修改。
