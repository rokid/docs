...
当这个能力可以问询时（retrievable）, 除了可以通过若琪控制: “帮我把灯调为红色”, 将 Rokid.Color 设置为红色, 还可以通过若琪问询: “这个灯现在是什么颜色”, 播报“现在这个灯是红色”。
当这个能力会主动推送状态时（proactivelyReported）, 那么就可以将这个终端状态作为一个自动化操作的触发器, 如一个自动化操作是 “当在早上家门关闭时, 关闭家里所有灯”, 那么就需要有一个“家门”的 Rokid.Switch 为 proactivelyReported, 当在早上它的值设为 off 时, 该自动化操作就会被触发。
