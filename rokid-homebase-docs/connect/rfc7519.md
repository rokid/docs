## 符合 RFC7519 的 JSON Web Token (JWT)
...
![](https://s.rokidcdn.com/homebase/upload/By5dxT8Az.png)

对应到厂商接入若琪的场景，可以将上图理解为以下步骤：
   ...
   (4)  厂商服务器接收到请求消息，通过对 JWT 签名进行解码和验证，读取 appId 和 userId，发送响应消息。
...
若琪通过 userId、appId、appSecret 签名得到：
...
厂商服务器判定 authorization.type 为 JWT 时，调用 jwt.decode 解码 token 得到 appId 和 userId，然后调用 jwt.verify 接口，验证 token 有效性并响应结果给若琪。
